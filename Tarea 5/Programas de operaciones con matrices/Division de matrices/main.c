...
int main()
{
   int MT1[10][10], MT2[10][10], MTCO[10][10];/*Declaracion de Matrices 1, 2  y el resultado de estas*/
   ...
   for (i = 0; i < filas; i++) {
      for (j = 0; j < columnas; j++)
        {
         MTCO[i][j] = MT1[i][j] / MT2[i][j];/*Operacion respectiva del programa*/
        }
   }

   /*Imprimir la matriz Cociente*/
   printf("El cociente de ambas matrices es:\n");/*Impresion del resultado de la suma de matrices*/
   for (i = 0; i < filas; i++) {
      for (j = 0; j < columnas; j++) {
         printf("%d ", MTCO[i][j]);
      }
